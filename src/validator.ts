function verifyTime(utcNotBefore?: string, utcNotOnOrAfter?: string): boolean {
  ...
  // Here setting time diff b/w notBeforeLocal and now can be greater than -5 minutes. This comes in the scenario when there is a mismatch of time between sp and idp server timings which result in ERR_SUBJECT_VALIDATION error
  return +now - +notBeforeLocal > -300000  && now < notOnOrAfterLocal;
}
